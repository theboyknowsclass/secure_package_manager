...
# Usage: .\dev-start.ps1 [service_name]
...
if ($Service) {
    Write-Host "🚀 Starting development service: $Service..." -ForegroundColor Green
} else {
...
if ($Service) {
    ...
    $serviceStatus = docker compose --env-file example.env.development -f docker-compose.base.yml -f docker-compose.dev.yml ps $Service --format "table {{.Service}}\t{{.Status}}"
    if ($serviceStatus -match "Up") {
        Write-Host "✅ Service '$Service' started successfully!" -ForegroundColor Green
        Write-Host "📋 Service status:" -ForegroundColor Cyan
        Write-Host $serviceStatus -ForegroundColor Cyan
    } else {
        Write-Host "❌ Service '$Service' failed to start. Check logs with:" -ForegroundColor Red
        Write-Host "   docker compose --env-file example.env.development -f docker-compose.base.yml -f docker-compose.dev.yml logs $Service" -ForegroundColor Yellow
        exit 1
    }
} else {
