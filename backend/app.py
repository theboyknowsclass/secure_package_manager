...
from models import db, User, Application, PackageRequest, Package, PackageValidation, AuditLog
...
def approve_package(package_id):
    """Approve a package for publishing"""
    try:
        ...
        audit_log = AuditLog(
            ...
            details=f'Package {package.name}@{package.version} approved'
        )
        db.session.add(audit_log)
        db.session.commit()
        
        return jsonify({'message': 'Package approved successfully'})
