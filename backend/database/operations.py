...
class DatabaseOperations:
    ...
    def update_package_status(self, package_id: int, new_status: str, 
                            status_model_class, **kwargs) -> bool:
        ...
        try:
            ...
            if self.is_flask_sqlalchemy:
                ...
                result = session.execute(
                    select(status_model_class).where(status_model_class.package_id == package_id)
                )
    ...
    def advance_package_status(self, package_id: int, next_status: str, 
                             status_model_class) -> bool:
    ...
    def create_package_records(self, packages_data: List[Dict[str, Any]], 
                             request_id: int, package_model_class, 
                             status_model_class, request_package_model_class) -> List[Any]:
    ...
    def log_action(self, user_id: int, action: str, resource_type: str, 
                  resource_id: int, details: str, audit_model_class) -> bool:
        """Log an audit action"""
        try:
    ...
    def count_packages_by_status(self, status: str, package_model_class, 
                               status_model_class) -> int:
        """Count packages by status"""
        try:
            if self.is_flask_sqlalchemy:
                return (package_model_class.query
                        .join(status_model_class)
                        .filter(status_model_class.status == status)
                        .count())
            else:
    ...
    def create_request_package_link(self, request_id: int, package_id: int, package_type: str, request_package_model_class) -> bool:
        """Create request-package link"""
        try:
            request_package = request_package_model_class(
                request_id=request_id,
