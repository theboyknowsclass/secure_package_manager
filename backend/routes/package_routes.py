...
from models import db, Application, PackageRequest, Package, PackageReference, AuditLog
import json
import logging
...
def get_package_security_scan_status(package_id):
    ...
    try:
        ...
        if not request.user.is_admin() and package.package_request.requestor_id != request.user.id:
            return jsonify({'error': 'Access denied'}), 403
...
def get_package_security_scan_report(package_id):
    ...
    try:
        ...
        if not scan_report:
            return jsonify({'error': 'No security scan report found for this package'}), 404
