...
class TestLicenseService(unittest.TestCase):
    ...
    def test_single_spdx_identifier_mit(self):
        ...
        with patch.object(
            ...
            result = self.license_service.validate_package_license(
                package_data
            )
    ...
    def test_and_expression(self):
        ...
        with patch.object(
            ...
            self.assertEqual(
                result["score"], 30
            )  # Should use worst license (GPL-3.0)
            self.assertEqual(len(result["errors"]), 0)
            self.assertGreater(
                len(result["warnings"]), 0
    ...
    def test_license_score_calculation(self):
        ...
        # Test always_allowed
        score = self.license_service._calculate_license_score(
            self.mock_mit_license
        )
        ...
        self.mock_cc0_license.status = "allowed"
        score = self.license_service._calculate_license_score(
            self.mock_cc0_license
        )
        self.assertEqual(score, 80)

        # Test avoid
        score = self.license_service._calculate_license_score(
            self.mock_gpl_license
        )
        self.assertEqual(score, 30)

        # Test blocked
        score = self.license_service._calculate_license_score(
            self.mock_blocked_license
        )
    ...
    def test_is_complex_license_expression(self):
        ...
        # Simple licenses
        self.assertFalse(
            self.license_service._is_complex_license_expression("MIT")
        )
        self.assertFalse(
        ...
        # Complex expressions
        self.assertTrue(
            self.license_service._is_complex_license_expression(
                "MIT OR Apache-2.0"
            )
        )
        self.assertTrue(
        ...
        self.assertFalse(
            self.license_service._is_complex_license_expression(None)
        )
