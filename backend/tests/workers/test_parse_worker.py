...
Uses real package-lock.json files from test_data directory. Tests the
core business logic in isolation without Flask app setup.
"""
...
class TestParseWorkerExtraction(unittest.TestCase):
    ...
    def test_extract_packages_simple_app(self):
        ...
        package_data = load_test_package_lock("simple_app")
        packages = ParseWorkerValidation.extract_packages_from_json(
            package_data
        )
    ...
    def test_extract_packages_duplicate_packages(self):
        ...
        # Should have 4 packages: root + 3 unique packages (lodash appears
        # twice)
        self.assertEqual(len(packages), 4)
        self.assertIn("", packages)  # Root package
        self.assertIn("node_modules/lodash", packages)
        ...
        self.assertIn(
            "node_modules/some-package/node_modules/lodash", packages
        )
    ...
    def test_extract_packages_adminportal(self):
        ...
        self.assertGreater(
            len(packages), 100
        )  # Adminportal has many dependencies
        self.assertIn("", packages)  # Root package
    ...
    def test_extract_package_name_from_real_files(self):
        ...
        package_name = ParseWorkerValidation.extract_package_name(
            "node_modules/lodash", lodash_info
        )
        ...
        package_name = ParseWorkerValidation.extract_package_name(
            "node_modules/@angular/core", angular_info
        )
        ...
        package_name = ParseWorkerValidation.extract_package_name(
            "node_modules/@types/lodash", types_info
        )
