...
Scanning), storing scan status/score.
"""
...
class SecurityWorker(BaseWorker):
    ...
    def _process_downloaded_packages(self) -> None:
        packages = self.ops.get_packages_by_status(
            "Downloaded", Package, PackageStatus
        )
    ...
    def _scan_single(self, package: Package, session) -> None:
        ...
        if result.get("status") == "failed":
            ...
            if "security_score" in result:
                package.package_status.security_score = result[
                    "security_score"
                ]
    ...
    def _mark_failed(self, package: Package) -> None:
        try:
            ...
            logger.exception(
                "Error marking package as failed in SecurityWorker"
            )
