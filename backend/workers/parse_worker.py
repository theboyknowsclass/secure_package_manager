...
class ParseWorker(BaseWorker):
    ...
    def _handle_stuck_requests(self) -> None:
        ...
        try:
            ...
            # to add a status field to Request
            stuck_requests = self.ops.get_pending_requests(Request)
            stuck_requests = [
                r for r in stuck_requests if r.created_at < stuck_threshold
            ]
    ...
    def _process_submitted_requests(self) -> None:
        ...
        try:
            ...
            # This is a simplified version - in practice, you might want to add
            # a status field to Request
            all_requests = self.ops.get_pending_requests(Request)
            logger.debug(
                f"Found {len(all_requests)} total requests in database"
            )
            ...
            if not requests_to_process:
                if not all_requests:
                    logger.info(
                        "ParseWorker heartbeat: No requests found in database to parse"
                    )
                else:
                    logger.info(
    ...
    def _filter_new_packages(
        ...
        for package_key, package_data in unique_packages.items():
            ...
            # Check if package already exists in database (deduplicated across
            # all requests)
            existing_package = self.ops.get_package_by_name_version(
                package_name, package_version, Package
            )

            if existing_package:
                # Check if this package is already linked to this request
                existing_link = self.ops.get_request_package_link(
                    request_id, existing_package.id, RequestPackage
                )

                if not existing_link:
                    # Create link between request and existing package
                    self.ops.create_request_package_link(
                        request_id,
                        existing_package.id,
                        "existing",
                        RequestPackage,
            ...
            package_data = {
                "name": package_name,
                "version": package_version,
                "npm_url": package_info.get(
                    "resolved"
                ),  # Map resolved URL to npm_url
                "integrity": package_info.get(
                    "integrity"
                ),  # Map integrity hash
                "license_identifier": package_info.get(
                    "license"
                ),  # Map license to license_identifier
            }
