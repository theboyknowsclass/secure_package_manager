...
repository. Processes packages that are approved but not yet published.
"""
...
class PublishWorker(BaseWorker):
    ...
    def process_cycle(self) -> None:
        ...
        try:
            ...
            logger.error(
                f"Error in package publishing cycle: {str(e)}", exc_info=True
            )

    def _handle_stuck_packages(self) -> None:
        """Handle packages that have been stuck in publishing state too
        long."""
        try:
            stuck_threshold = datetime.utcnow() - self.stuck_package_timeout
            session = self.ops._get_session()

            # Get packages with stuck publish status - need to query directly
            # since there's no specific method
            stuck_packages = (
    ...
    def _process_pending_publishing(self) -> None:
        ...
        try:
            ...
            # Get packages that need publishing - approved packages with
            ...
            logger.error(
                f"Error in _process_pending_publishing: {str(e)}",
                exc_info=True,
            )
            session = self.ops._get_session()
            session.rollback()
    ...
    def _publish_single_package(self, package: Package) -> None:
        ...
        try:
            # Update publish status to publishing (keep main status as
            # Approved)
            if package.package_status:
            ...
            if success:
                # Mark as published successfully (keep status as Approved, set
                # published_at and publish_status)
                if package.package_status:
                ...
                logger.info(
                    (
                        f"Successfully published package "
    ...
    def _mark_package_publish_failed(
        ...
        try:
            if package.package_status:
                ...
                package.package_status.updated_at = datetime.utcnow()
            logger.error(
                f"Package {package.name}@{package.version} publish failed: {error_message}"
            )
        except Exception as e:
    ...
    def retry_failed_publishing(self) -> Dict[str, Any]:
        ...
        try:
            with self.db_service.get_session() as session:
                ...
                logger.info(
                    f"Retried {retried_count} failed publishing packages"
                )
                return {
