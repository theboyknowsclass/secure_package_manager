...
class PackageStatusMonitor:
    ...
    def get_package_counts_by_status(self) -> Dict[str, int]:
        ...
        try:
            with SessionHelper.get_session() as db:
                ...
                total_query = text("SELECT COUNT(*) as total FROM package_status")
            ...
            with SessionHelper.get_session() as db:
                ...
                for row in result:
                    status_breakdown[row.status] = {
                        'count': row.count,
                        'oldest_created': row.oldest_created.isoformat() if row.oldest_created else None,
                        'newest_updated': row.newest_updated.isoformat() if row.newest_updated else None
                    }
                ...
                stuck_packages = {row.status: row.count for row in stuck_result}
    ...
    def print_status_report(self, counts: Dict[str, int], format_type: str = "table") -> None:
    ...
    def run_continuous(self, interval: int = 60, format_type: str = "table") -> None:
        ...
        if args.detailed:
            ...
            if breakdown:
                if args.output == "json":
                    ...
                    print(f"Status Types: {breakdown['summary']['total_statuses']}")
                    print(f"Stuck Packages: {breakdown['summary']['stuck_count']}")
                    print("\nStatus Breakdown:")
