...
class AdminAPITester:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if packages:
                    ...
                    required_fields = ['id', 'name', 'version', 'security_score', 'license_score', 'license_identifier']
    ...
    def test_repository_config(self) -> bool:
        ...
        try:
            headers = {"Authorization": f"Bearer {self.token}"}
            response = self.session.get(
                f"{BASE_URL}/api/admin/repository-config", 
                headers=headers,
                timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                print(f"✅ Repository config endpoint working - found {len(configs)} configs")
                ...
                if expected_keys.issubset(actual_keys):
                    ...
                    print(f"⚠️  Missing expected config keys. Expected: {expected_keys}, Found: {actual_keys}")
            ...
            if response.status_code == 200:
                ...
                if licenses:
                    ...
                    if expected_identifiers.intersection(actual_identifiers):
                        ...
                        print(f"⚠️  Missing expected license identifiers. Expected: {expected_identifiers}, Found: {actual_identifiers}")
    ...
    def test_config_status(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"✅ Config status endpoint working - complete: {is_complete}, missing: {missing_keys}")
                return True
            else:
                print(f"❌ Config status endpoint failed: {response.status_code} - {response.text}")
                return False
        except Exception as e:
